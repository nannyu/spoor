...
describe('i18n 国际化模块', () => {
  ...
  describe('插值功能', () => {
    ...
    it('agents 沙盒标题插值正常', () => {
      i18n.changeLanguage('en');
      expect(i18n.t('agents.sandbox_title', { name: 'Challenger' })).toBe('Sandbox: Challenger');
    });

    it('agents 沙盒空消息插值正常', () => {
      i18n.changeLanguage('zh');
      expect(i18n.t('agents.sandbox_empty', { name: '挑战者' })).toBe(
        '正在测试 挑战者 的沙盒。发送消息以查看其根据当前系统提示词的响应。'
      );
    });
  });
