...
export function NoteLayoutGlass({ node, editingNodeId, setEditingNodeId }: NodeContentProps) {
  ...
  return (
    <div className="note-surface-glass isolate relative flex h-full w-full flex-col overflow-hidden rounded-xl outline-none transition-all duration-500 [transform:translateZ(0)]">
      <>
        <div className="note-glass-wash pointer-events-none absolute inset-0" aria-hidden />
        <div className="note-glass-hotspot pointer-events-none absolute left-0 top-0 h-24 w-24 blur-md" aria-hidden />
      </>
      <div className="relative z-10 flex min-h-0 flex-1 flex-col gap-4 p-6">
        <header className="note-chrome flex shrink-0 items-center justify-between">
