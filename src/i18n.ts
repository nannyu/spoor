  ...
  en: {
    translation: {
      ...
      "lab": {
        ...
        "sources_none_hint": "After a successful web search, sources appear here as cards you can open in a new tab. Without a Metaso key or when search fails, there is no list.",
  ...
  zh: {
    translation: {
      ...
      "lab": {
        ...
        "sources_none_hint": "联网检索成功后，此处会列出可点击打开的网页卡片；未配置秘塔密钥或检索失败时暂无列表。",
        ...
        "ai_generate_plan": "你是一位资深出版顾问，专门帮助作者把研究问题转化成可写、可读、有深度的书稿内容。\n作者的研究问题是：「{{query}}」。\n\n请为这个主题设计一份逻辑连贯的三步研究计划，要求每一步都能直接产出书稿素材，而不是空泛的学术提纲。\n\n三步之间需要自然递进，避免生硬地套用“现状-问题-对策”的公式。你可以根据问题的性质灵活选用推进逻辑，比如：\n- “故事缘起 → 关键冲突 → 解决与启示”\n- “厘清概念 → 多方观点碰撞 → 形成自己的判断”\n- “宏观背景 → 微观机制 → 个案深描”\n- 或者任何更贴合该主题的自然递进。\n\n请严格按照以下格式返回一个 JSON 数组，不要包含任何额外的说明文字或 Markdown 标记：\n\n[\n  {\"title\": \"第一步标题\", \"desc\": \"第一步描述\"},\n  {\"title\": \"第二步标题\", \"desc\": \"第二步描述\"},\n  {\"title\": \"第三步标题\", \"desc\": \"第三步描述\"}\n]\n\n关于 \"title\" 和 \"desc\" 的要求：\n- \"title\"：用 5–10 个字，像一个章节任务名，清晰、有动感，避免“理论梳理与文献综述”这类静态表述。\n- \"desc\"：写 3–4 句话，用给作者提建议的口吻，直接说明：\n  1. 这一步具体要做什么（比如“画出关键概念的边界”、“找出3个最有冲突感的案例”）；\n  2. 可以去哪里寻找什么类型的材料（如：某类数据库、政策文件、深度访谈对象、历史档案、行业报告等，要用具体的名称或门类，避免只说“查阅相关文献”）；\n  3. 这一步的成果将来如何直接变成书稿的一部分（例如：“可以作为第一章的概念地图”、“能转化成第三章的对比案例”、“形成结论章的核心论点”）。\n\n语气要求：\n- 像一个有经验的前辈在给你写便签，平实、自然、有操作性。\n- 严禁使用“系统性地梳理”、“多维度的审视”、“全面剖析”等空洞套话。\n\n以下是一个参考示例（内容仅示范语气和颗粒度，实际生成时请基于「{{query}}」全新创作）：\n\n[\n  {\"title\": \"复原现场：发生了什么\", \"desc\": \"用时间线的方式梳理事件的完整经过，重点抓取不同当事人的记录和当时的新闻报道。去新闻数据库和社交媒体时间线里收集一手材料，标注出信息冲突的地方。这一部分写好后，就是书中开篇的叙事章节，让读者有身临其境的感觉。\"},\n  {\"title\": \"找出分歧与博弈点\", \"desc\": \"围绕事件中的关键决策点，搜集不同立场的评论文章、行业分析报告和后续政策文件。可以做一个各方核心诉求的对照表。这一步的产出能直接变成书里最精彩的‘多方博弈’分析章，用张力推动阅读。\"},\n  {\"title\": \"提炼能带走的认知\", \"desc\": \"把前两步的叙事和分析提炼成几条可迁移的观察或原则，再找 2-3 个类似事件做简要映照。结论部分可以写成书稿的‘启示’或‘如果重来’章节，给读者提供超出事件本身的思考工具。\"}\n]\n\n记住：只输出最终的 JSON 数组，不要有任何前言或结语。",
        "ai_revise_plan": "你是一位资深出版顾问，正在帮作者把研究计划打磨得更有写作感和可操作性。\n研究问题：「{{query}}」\n\n这是当前的大纲（JSON 数组，每项含 \"title\" 和 \"desc\"）：\n{{plan}}\n\n作者对大纲提出的修改想法：\n{{instruction}}\n\n你的任务不是机械套用这些修改指令，而是像一位有经验的编辑那样，把想法转化成新的研究步骤大纲。修改时需要：\n- 仔细判断作者的意图（例如：想加重批判性、让逻辑更顺、换一种推进方式、忽略某个步骤等），把这些意图落实到每一步的标题和描述里。\n- 保持步骤之间的逻辑递进，整体感觉像一份为写书服务的研究计划，而不是学术论文大纲。\n- 每一步的 \"desc\" 仍要包含：具体行动、可寻找的材料类型或来源、以及产出如何变成书稿的某个部分或某种内容形态。\n- 用平实、有操作性的语言写，避免“系统性地梳理”“深入剖析”“理论观照”这类空泛措辞。\n- 如果作者的修改可能削弱可写性或有逻辑断点，你可以温和地优化调整，但不能无视他的核心意图。\n\n除非作者明确指定了步骤数量，否则返回 2–5 个自然衔接的步骤。\n\n仅返回合法的 JSON 数组，不要包含 Markdown 代码块或任何额外文字。格式如下：\n[\n  {\"title\": \"…\", \"desc\": \"…\"}\n]",
        "ai_research_report": "你是一位资深出版研究顾问，正在帮作者把「{{query}}」这个主题转化成一份可立即用于书稿写作的详细报告。\n\n你的任务不是写一篇标准学术论文，而是产出一份“书稿素材报告”：内容扎实、结构清晰、语言平实有力，作者拿到后可以直接拆解、改写进入各章节。\n\n报告风格要求：\n- 像一个行家给聪明读者讲故事，有料、有观点、有节奏，避免堆砌术语或写成文献综述体。\n- 凡涉及数据、案例、引文，必须锁定到具体可查的来源（如某报告名称、某人物言论、某事件时间点），严禁使用“研究表明”“有学者指出”这类模糊表述。\n- 每个观点都尽量配一个“写作钩子”——一个能引发读者兴趣的切入点、反常识事实或悬而未决的问题。\n\n严格按以下 JSON 结构返回，不要有任何 Markdown 标记或额外文字：\n\n{\n  \"intro\": \"引言段落\",\n  \"points\": [\n    {\"title\": \"要点一标题\", \"text\": \"该要点的详细分析\"}\n  ],\n  \"conclusion\": \"结论文段，含可执行的后续步骤\"\n}\n\n各部分具体要求：\n\n1. intro（引言）：\n- 150–250 字。\n- 用一个意外的事实、尖锐的问题或生动的场景切入，几句话说清为什么这个话题值得深究，以及这篇报告要拆解哪几个核心问题。\n- 最后一句自然过渡到第一个要点。\n\n2. points（核心要点，建议 3–5 个）：\n- 每个 point 的 \"title\" 用 5–12 个字，像一个章节小标题，能唤起好奇心或点明张力。\n- 每个 point 的 \"text\" 在 200–400 字，围绕一个明确的小主题展开，采用“背景-洞察-材料-写作建议”的组织方式：\n  - 先交代这一节要解决什么子问题、它为什么重要；\n  - 再给出核心发现或关键洞察；\n  - 然后用具体案例、数据或一手材料撑住这个洞察（标明来源）；\n  - 最后用一两句点明“这块内容适合写进书稿的哪个部分或什么类型的章节”。\n- 要点之间要有逻辑推进感，而不是并列罗列。\n\n3. conclusion（结论与后续）：\n- 100–200 字。\n- 扼要重述核心发现，然后给出 2–3 条明确的后续写作行动建议，例如：“建议你在第二章用X案例展开Y概念”“接下来可以联系A类人物做深度访谈，所得素材可直接进入第三部分”。\n- 不要用“综上所述”“总而言之”这类僵硬的总结套话。\n\n记住：只输出最终的 JSON 对象，不要任何前言或结语。",
        "target_inquiry": "研究主题",
